...
**В-третьих**, универсальная инфраструктура маршрутизации не может быть автоматически *[федерализована][FED]*. Она предназначена для работы в рамках одной *[hub-and-spoke][SPOKEHUB]* архитектуры, и, как только появится необходимость выйти за ее пределы, вам придется предоставлять дополнительную информацию, а именно в форме ответа на вопрос *"что это за шаблон?"*. Посмотрите на механизмы федерализации, реализованные разными продуктами поверх AMQP. В них во всех присутствует флаг определяющий шаблон, явно или неявно (путем поддержки только одного шаблона).
